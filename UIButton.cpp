...
UIButton::UIButton(int x, int y, int w, int h, bool	bang, SDL_Surface *aSource,  SDL_Surface *aDestination) {
    //Set the button's attributes
    box.x = x;
    box.y = y;
    box.w = w;
...
bool UIButton::getBangButton() {
	return bangButton;
}
void UIButton::setBangButton(bool set) {
	bangButton = set;
...
bool UIButton::handle_events(SDL_Event &event) {
	...
	if (!buttonSet) {
		...
		if( (event.type == SDL_MOUSEBUTTONUP) && (bangButton) )
		{
			//If the left mouse button was released
			if( event.button.button == SDL_BUTTON_LEFT )
			{
